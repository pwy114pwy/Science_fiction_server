...
router.post('/api/AddBook', async (req, res) => {
  try {
    const { Author_ID,Author_Introduce } = req.body;

    if (Author_ID) { // 更新操作
      // 更新书籍信息
      await pool.query(
        'UPDATE author SET Author_Name = ?, Tag = ?, Topic = ?, Author_ID = ?, Rating = ?, Book_Introduce = ?, Img_Url = ? WHERE Book_ID = ?',
        [Book_Name, Tag, Topic, Author_ID, Rating, Book_Introduce, Img_Url, Book_ID]
      );
