...
router.post('/api/AddBook', async (req, res) => {
  try {
    const { Author_ID, Author_Name, Author_Introductipn } = req.body;

    if (Author_ID) { // 更新操作
      // 更新书籍信息
      await pool.query(
        'UPDATE author SET Author_Name = ?,Author_Introduce = ? WHERE Author_ID = ?',
        [Author_Name, Author_Introduce, Author_ID]
      );
      ...
      // 插入新作者
      await pool.query(
        'INSERT INTO author (Author_Name, Tag, Topic, Author_ID, Rating, Book_Introduce, Img_Url) VALUES (?, ?, ?, ?, ?, ?, ?)',
