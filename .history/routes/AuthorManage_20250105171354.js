...
// 删除作者
router.post('/api/DelAuthor', async (req, res) => {
  try {
    const { Author_ID } = req.body;
    
    if (!Author_ID) {
      return res.status(400).json({ error: '作者ID是必填的' });
    }
    ...
    if (deleteResult.affectedRows > 0) {
      return res.json({ message: '作者删除成功' });
    } else {
      return res.status(404).json({ error: '没有找到要删除的zuoez' });
    }
  } catch (error) {
