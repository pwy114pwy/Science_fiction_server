...
router.post('/api/Addcharacter', async (req, res) => {
  try {
    const {   Character_IDIllustrated_ID, Illustrated_Name, Role_Name, X_Position, Y_Position, Role_Introduction, Role_Image, Tag,Classic_Quotes,ContributeList,AbilityList } = req.body;


    if (Book_ID) { // 更新操作
      // 更新书籍信息
      await pool.query(
        'UPDATE book SET Book_Name = ?, Tag = ?, Topic = ?, Author = ?, Rating = ?, Book_Introduce = ?, Img_Url = ? WHERE Book_ID = ?',
