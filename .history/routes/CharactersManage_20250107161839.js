...
router.post('/api/Addcharacter', async (req, res) => {
  try {
    ...
    let Power_Ability1=ContributeList[0]
    let Power_Ability2=ContributeList[1]
    let Power_Ability3=ContributeList[2]
    let Power_Ability4=ContributeList[3]
    let Power_Ability5=ContributeList[4]
    let Power_Ability6=ContributeList[5]
    console.log(req.body)
    // if (Character_ID) { // 更新操作
    //   // 更新角色信息
    //   await pool.query(
    //     'UPDATE book SET Illustrated_ID = ?, Illustrated_Name = ?, Role_Name = ?, X_Position = ?, Y_Position = ?, Role_Introduction = ?, Role_Image = ? , Tag = ?,Classic_Quotes = ?, WHERE Book_ID = ?',
