...
router.post('/api/Addcharacter', async (req, res) => {
  try {
    ...
    if (Character_ID) { // 更新操作
      // 更新角色信息
      await pool.query(
        `UPDATE char SET Illustrated_ID = ?, Illustrated_Name = ?, Role_Name = ?, X_Position = ?, Y_Position = ?,
      ...
      // 检查角色是否已存在
      let [existingBooks] = await pool.query('SELECT * FROM book WHERE Role_Name = ?', [Book_Name]);
