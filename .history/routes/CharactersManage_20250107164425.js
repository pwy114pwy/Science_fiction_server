...
router.post('/api/Addcharacter', async (req, res) => {
  try {
    const { Character_ID, Illustrated_Name, Role_Name, X_Position, Y_Position, Role_Introduction, Role_Image, Tag, Classic_Quotes, ContributeList, AbilityList } = req.body;
    let Illustrated_ID;
    let [character_Name]=await pool.query('select Illustrated_ID from character')
