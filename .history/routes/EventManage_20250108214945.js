...
router.post('/api/AddRelationship', async (req, res) => {
  try {
    ...
    if (ImportantEvent_ID) { // 更新操作
      ...
      // 插入新事件
      await pool.query(
        `INSERT INTO event (Illustrated_ID,Illustrated_Name,Event_Time,Event_Description,To_CharacterID,To_Name,Relation_Type) VALUES (?,?,?,?,?,?,?)`,
          [Illustrated_ID,Illustrated_Name,From_CharacterID,From_Name,To_CharacterID,To_Name,Relation_Type]
      );
