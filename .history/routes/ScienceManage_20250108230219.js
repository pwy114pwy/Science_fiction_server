...
router.post('/api/getScien', async (req, res) => {

  try {
    let { Illustrated_ID } = req.body;
    const [rows] = await pool.query('SELECT * FROM worldview WHERE Illustrated_ID = ?', [Illustrated_ID]);
    res.json(rows);
  } catch (error) {
...
// 添加或更新关系
router.post('/api/addWorldview', async (req, res) => {
  try {
