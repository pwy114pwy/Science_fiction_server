...
router.post('/api/addScience', async (req, res) => {
  try {
    ...
    if (Science_ID) { // 更新操作
      // 更新关系信息
      await pool.query(
        `UPDATE science SET Illustrated_ID=?,Illustrated_Name=?, Worldview_Title=?,Worldview_Description=?
          WHERE Worldview_ID = ?`,
      ...
      // 插入新事件
      await pool.query(
        `INSERT INTO science (Illustrated_ID,Illustrated_Name,Worldview_Title,Worldview_Description) VALUES (?,?,?,?)`,
