...
router.post('/api/addScience', async (req, res) => {
  try {
    const { Science_ID, Illustrated_ID, Illustrated_Name, Science_Name, Science_Image,
    ...
    if (Science_ID) { // 更新操作
      // 更新关系信息
      await pool.query(
        `UPDATE science SET Illustrated_ID=?,Illustrated_Name=?, Science_Name=?,Science_Image=?,
        Science_description=?,Science_Rating_Strength=?,Science_Rating_Speed=?,
        Science_Rating_Intelligence=?,Science_Rating_Durability=?
          WHERE Science_ID = ?`,
        [Illustrated_ID, Illustrated_Name, Science_Name, Science_Image,
          Science_description,Science_Rating_Strength,Science_Rating_Speed,
      ...
      // 插入新事件
      await pool.query(
        `INSERT INTO science (Illustrated_ID, Illustrated_Name, Science_Name, Science_Image,
          Science_description,Science_Rating_Strength,Science_Rating_Speed,
...
router.post('/api/delScience', async (req, res) => {
  try {
    const { Science_ID } = req.body;

    if (!Science_ID) {
      return res.status(400).json({ error: '科技观ID是必填的' });
    }

    const [deleteResult] = await pool.query(
      'DELETE FROM science WHERE Science_ID = ?',
      [Science_ID]
    );

    if (deleteResult.affectedRows > 0) {
      return res.json({ message: '科技观删除成功' });
    } else {
      return res.status(404).json({ error: '没有找到要删除的科技观' });
    }
  } catch (error) {
