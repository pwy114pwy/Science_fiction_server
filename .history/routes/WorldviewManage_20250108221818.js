...
router.post('/api/addEvent', async (req, res) => {
  try {
    const { Worldview_ID, Illustrated_ID, Illustrated_Name,Event_Time,Event_Description,Event_Value  } = req.body;

    if (Worldview_ID) { // 更新操作
      // 更新关系信息
      await pool.query(
        `UPDATE event SET Worldview_ID=?,Illustrated_Name=?, Event_Time=?,Event_Description=?,Event_Value=?
          WHERE Worldview_ID = ?`,
      ...
      // 插入新事件
      await pool.query(
        `INSERT INTO event (Worldview_ID,Illustrated_Name,Event_Time,Event_Description,Event_Value) VALUES (?,?,?,?,?)`,
