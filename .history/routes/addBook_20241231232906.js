...
router.post('/api/AddBook', async (req, res) => {
  try {
    ...
    if (existingBooks.length > 0) {
      ...
      //查找作者ID
      const [authorID] = await pool.query(' INTO book (Book_Name, Tag, Topic, Author, Rating, Book_introduce,Img_Url) VALUES (?,?, ?, ?, ?, ?)', [Book_ID, Book_Name, Comment_User_Name, Comment_User_NickName, Comment_Time, Comment_Content]);
