...
router.post('/api/AddBook', async (req, res) => {
  try {
    ...
    if (existingBooks.length > 0) {
      ...
      //查找作者ID
      const [authorID] = await pool.query('SELECT Authod_ID FROM author WHERE A', [Book_ID, Book_Name, Comment_User_Name, Comment_User_NickName, Comment_Time, Comment_Content]);
