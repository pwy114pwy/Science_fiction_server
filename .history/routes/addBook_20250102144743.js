...
router.post('/api/AddBook', async (req, res) => {
  try {
    ...
    if (existingBooks.length > 0) {
      ...
      // 添加书籍
      const [result] = await pool.query('INSERT INTO book (Book_Name, Tag, Topic, Author_ID, Author, Rating, Book_Introduce,Img_Url) VALUES (?,?,?,?, ?, ?, ?, ?)', [Book_Name, Tag, Topic,authorID[0].Author_ID ,Author, Rating, Book_ntroduce,Img_Url]);
