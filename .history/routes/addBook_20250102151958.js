...
// 添加或更新书籍
router.post('/api/Addook', async (req, res) => {
  try {
    const { id, Book_Name, Tag, Topic, Author, Rating, Book_Introduce, Img_Url } = req.body;
    ...
    if (id) { // 更新操作
      // 更新书籍信息
      await pool.query(
        'UPDATE book SET Book_Name = ?, Tag = ?, Topic = ?, Author_ID = ?, Rating = ?, Book_Introduce = ?, Img_Url = ? WHERE id = ?',
