...
router.post('/api/GetDetailBook', validateBookId, async (req, res) => {
  try {
    ...
    //查找书籍评论
    const [commentRows] = await pool.query('SELECT Author_Introduction FROM author WHERE Author_Name = ?', [book.Author]);
    let bookIntroduction = null;
    if (commentRows.length > 0) {
      bookIntroduction = authorRows[0].Author_Introduction;
    }
