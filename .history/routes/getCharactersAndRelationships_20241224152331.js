...
router.post('/api/getCharactersAndRelationships', async (req, res) => {
  ...
  try {
    ...
    const [characters] = await pool.query(
      'SELECT * CharacterID, Name, NovelID FROM Characters WHERE Illustrated_ID = ?',[Illustrated_ID]
    );

    // 查询所有关系
    const [relationships] = await pool.query(
      'SELECT * FromCharacterID, ToCharacterID, RelationType FROM Relationships WHERE Illustrated_ID = ?',
      [Illustrated_ID]
    );
