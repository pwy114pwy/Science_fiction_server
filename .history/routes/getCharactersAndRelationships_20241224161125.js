...
router.post('/api/getCharactersAndRelationships', async (req, res) => {
  try {
    ...
    const [characters] = await pool.query(
      'SELECT Character_ID, Role_Name, Illustrated_ID, X_Position, _position FROM characters WHERE Illustrated_ID = ?',
      [Illustrated_ID]
    );

    // 查询所有关系
    const [relationships] = await pool.query(
      'SELECT From_CharacterID, To_CharacterID, Relation_Type FROM relationships WHERE Illustrated_ID = ?',
      [Illustrated_ID]
    );
    ...
    const formattedRelationships = relationships.map((relation) => ({
      from: relation.From_CharacterID, // ECharts索引从0开始
      to: relation.To_CharacterID,
