...
router.post('/api/AddDiscussion', async (req, res) => {
  try {
    ...
    // 添加讨论区信息
    // let arr = []
    const [result] = await pool.query('INSERT INTO comment (Book_ID,Book_Name, 
      Comment_User_Name, Comment_User_NickName, Comment_Time,Comment_Content) VALUES (?,?, ?, ?, ?, ?)', [Book_ID, Book_Name, Comment_User_Name, Comment_User_NickName, Comment_Time, Comment_Content]);
    // arr = discussion
