...
router.post('/api/deleteDiscussion', async (req, res) => {
  try {
    ...
    // 检查评论是否已存在
    const [existingComments] = await pool.query('SELECT * FROM discussion WHERE Book_Comment_ID = ?', [Discussion_ID]);
    if (existingUsers.length > 0 && existingComments.length > 0) {
      //删除评论
      const [deleteResult] = await pool.query(
        'DELETE FROM discussion WHERE Comment_User_Name = ? AND Book_Comment_ID = ?',
