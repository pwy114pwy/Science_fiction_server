...
//获取所有讨论
router.get('/api/getAllDiscussion', async (req, res) => {
  try {
    const [discussion] = await pool.query('SELECT * FROM discussion');
    res.json(discussion);
  } catch (error) {
    console.error(error);
...
router.post('/api/deleteDiscussion', async (req, res) => {
  try {
    const { Discussion_ID, Discussion_User_ID } = req.body;

    if (!Discussion_ID || !Discussion_User_ID) {
      return res.status(400).json({ error: '所有字段都是必填的' });
    }

    // 检查用户是否已存在
    const [existingUsers] = await pool.query('SELECT * FROM user WHERE User_ID = ?', [Discussion_User_ID]);
    // 检查评论是否已存在
    const [existingComments] = await pool.query('SELECT * FROM discussion WHERE Discussion_ID = ?', [Discussion_ID]);
    if (existingUsers.length > 0 && existingComments.length > 0) {
      //删除评论
      const [deleteResult] = await pool.query(
        'DELETE FROM discussion WHERE Discussion_User_ID = ? AND Discussion_ID = ?',
        [Discussion_User_ID, Discussion_ID]
      );
      if(deleteResult.affectedRows==0){
        return res.status(400).json({ error: '删除评论失败' });
      }
      console.log(deleteResult)
      return res.json({ success: true, message: '删除toal成功' });
