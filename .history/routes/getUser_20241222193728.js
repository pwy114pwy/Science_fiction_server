...
router.post('/api/user/change_userinfo', async (req, res) => {
  try {
    const { username, new_nickname } = req.body;
...
WHERE condition;
    ...
    const [result] = await pool.query('UPDATE INTO user (User_Name, Email, PassWord, NickName) VALUES (?, ?, ?, ?)', [username, email, hashedPassword, nickname]);
