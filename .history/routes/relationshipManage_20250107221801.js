...
router.post('/api/getRelationsh', async (req, res) => {

  try {
    let { Relationship_ID } = req.body;
    const [rows] = await pool.query('SELECT * FROM relationships WHERE Relationship_ID = ?', [Relationship_ID]);
    res.json(rows);
  } catch (error) {
