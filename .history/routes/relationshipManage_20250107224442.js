...
router.post('/api/AddRelationship', async (req, res) => {
  try {
    ...
    let [from_ID]=await pool.query("SELECT From_CharacterID FROM characters WHERE Role_Name = ?",[From_CharacterID])
    let [to_ID]=await pool.query("SELECT From_CharacterID FROM characters WHERE Role_Name = ?",[From_CharacterID])

    if (Relationship_ID) { // 更新操作
      // 更新关系信息
      await pool.query(
        `UPDATE relationships SET From_CharacterID=?,From_Name=?,To_CharacterID=?,To_Name=?,Relation_Type=?
          WHERE Relationship_ID = ?`,
        [From_CharacterID,From_Name,To_CharacterID,To_Name,Relation_Type,Relationship_ID]
      );
      return res.json({ success: true, message: '关系更新成功' });
    } else { // 添加操作
      // 插入新关系
      await pool.query(
        `INSERT INTO relationships (From_CharacterID,From_Name,To_CharacterID,To_Name,Relation_Type) VALUES (?,?,?,?,?)`,
