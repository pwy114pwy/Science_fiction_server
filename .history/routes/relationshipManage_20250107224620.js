...
router.post('/api/AddRelationship', async (req, res) => {
  try {
    ...
    let [from_ID]=await pool.query("SELECT Character_ID FROM characters WHERE Role_Name = ?",[From_CharacterID])
    let [to_ID]=await pool.query("SELECT Character_ID FROM characters WHERE Role_Name = ?",[To_CharacterID])
