...
router.post('/api/AddBookComment', async (req, res) => {
  try {
    ...
    if (existingUsers.length > 0 && existingBooks.length > 0) {
      // 插入评论
      const [result] = await pool.query('INSERT INTO comment (Book_Name, Comment_User_Name, Comment_User_NickName, Comment_Time) VALUES (?, ?, ?, ?)', [username, email, hashedPassword, nickname]);
